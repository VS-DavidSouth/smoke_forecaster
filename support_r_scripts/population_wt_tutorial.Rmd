...
Plot of smoke during October 11th. 
...
ggplot(bluesky_grid) +
  geom_sf(aes(fill=pm)) +
    scale_fill_gradient2(name="Smoke PM2.5", 
      low="black", mid="#89fffd", high="#ef32d9", midpoint = 200) +
  geom_sf(data=cali_county, color = "red", alpha = 0)
# looks okay


bluesky_grid <- bluesky_grid %>% 
  left_join(cal_population, by = "id")

# plot popluation density to see if it looks correct
ggplot(bluesky_grid) +
  geom_sf(aes(fill = popden)) +
  geom_sf(data=cali_county, aes(color = "red"), alpha = 0)
...
pm_mat <- as.matrix(cbind(cali_mean_pm_smk$pm, cali_mean_pm_smk$pm))
